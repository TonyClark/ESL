...
public abstract class List<Element> implements Serializable {
  ...
  public List<Element> appendRec(List<Element> list) {
    if (isNil())
      return list;
    else if (list.isNil())
      return this;
  ...
  public List<Element> append(List<Element> list) {
    if (isNil())
      ...
      Cons<Element> append = null;
      Cons<Element> result = null;
      List<Element> l = this;
      while (!l.isNil()) {
        if (append == null) {
          append = new Cons<Element>(null, null);
          result = append;
        } else {
          Cons<Element> cons = new Cons<Element>(null, null);
          append.setTail(cons);
          append = cons;
        }
        append.setHead(l.getHead());
        l = l.getTail();
      }
      append.setTail(list);
      return result;
