...
public class Act extends AST implements DecContainer, RefContainer {
	...
	// Actors have a behaviour that defines how they process messages from their
	// queue.
	...
	private Fun armsToFun(String name, Vector<BArm> namedArms) {
		...
		for (int i = 0; i < arity; i++) {
			PVar var = (PVar) term.getPatterns()[i];
			args[i] = new Dec(start, end, path, var.getName(), var.getDeclaredType(), var.getDeclaredType());
		}
			...
			for (Binding b : bindings) {
				if (b.getName().equals(exports.getNames()[i].getName())) {
					if (b.isTypeBinding())
						...
						refs[i] = new ast.binding.Var(getLineStart(), getLineEnd(), b.getName(), b.getDeclaredType(), null);
				}
			}
	...
	private Dec[] getExportedTypes(Env<String, Type> env) {
		...
		for (Binding b : Binding.valueBindings(bindings)) {
			...
			if (exports.contains(b.getName())) {
				...
				decs[decIndex++] = new Dec(b.getLineStart(), b.getLineEnd(), b.getPath(), b.getName(), bType, bType);
			}
		}
		return decs;
	}
	...
	private int getFirstHandlerLine() {
		if (arms.length == 0)
			...
			return arms[0].patterns[0].getLineStart();
	}
		...
		for (ExportedName export : exports.getNames()) {
			if (export != null) {
				...
				if (dynamic == null)
					...
					newExports[dynamic.getIndex()] = export;
			}
		}
	...
	private Type selfType(Env<String, Type> env) {
		...
		for (BArm a : arms) {
			...
			if (!names.contains(term.name)) names.add(term.name);
		}

		MessageType[] handlers = new MessageType[names.size()];
		int[] index = new int[] { 0 };
		...
		for (BArm a : arms) {
			...
			if (!names.contains(term.name)) {
				names.add(term.name);
				Pattern.types(a.patterns, env, (newEnv, types) -> {
