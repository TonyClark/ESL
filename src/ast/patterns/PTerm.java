...
public class PTerm extends Pattern {
	...
	public Env<String, Type> bind(Env<String, Type> env, Type type) {
		if (type instanceof ast.types.Term) {
			...
			ast.types.Term term = union.getTerm(name, patterns.length);
			if (term == null)
				return null;
			else {
				for (int i = 0; i < patterns.length; i++) {
			...
			if (env.binds(name)) {
				...
				if (isGeneric()) {
						...
						if (fun.getNames().length == types.length) {
							...
							if (type instanceof Cnstr) {
								...
								if (Type.equals(suppliedType, expectedType, env)) {
									...
									throw new TypeMatchError(getLineStart(), getLineEnd(), suppliedType, expectedType);
							} else
								throw new TypeError(getLineStart(), getLineEnd(), "expecting a constructor " + name);
						} else
							throw new TypeError(getLineStart(), getLineEnd(), "arity mismatch " + name);
					} else
						throw new TypeError(getLineStart(), getLineEnd(), "expecting a type function " + name);
				} else {
					...
					if (type instanceof Cnstr) {
						...
						throw new TypeError(getLineStart(), getLineEnd(), "expecting a constructor: " + name + ":" + type);
				}
			} else if (isGeneric()) {
