...
public class Binding extends Dec {
  ...
  private static AST merge0ArityFunctions(String name, Vector<FunBind> fs) {
    ...
    return new Fun(fs.get(0).getLineStart(), fs.get(0).getLineEnd(), fs.get(0).getPath(), new Str(name), new Dec[] {}, fs.get(0).getType(), merge0ArityFunctions(name, fs, 0), false);
  }
  ...
  private static AST mergeFunctions(String name, Vector<FunBind> fs) {
    ...
    if (arity == 0)
      ...
      return new Fun(fs.get(0).getLineStart(), fs.get(0).getLineEnd(), fs.get(0).getPath(), new Str(name), args, fs.get(0).getType(), new Case(fs.get(0).getLineStart(), fs.get(0).getLineEnd(), new Dec[] {}, vars, arms), false);
    }
  }
