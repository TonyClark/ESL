...
@BoaConstructor(fields = { "name", "declaredType", "sourceType", "value" })
...
public class Binding extends Dec {
	...
	private static AST merge0ArityFunctions(String name, Vector<FunBind> fs, int i) {
		if (i == fs.size())
			...
			return new If(fs.get(i).getLineStart(), fs.get(i).getLineEnd(), fs.get(i).guard, fs.get(i).body, merge0ArityFunctions(name, fs, i + 1));
	}
	...
	private static Binding mergeBinding(Binding[] bindings, String name) {
		...
		for (Binding b : bindings) {
			...
			if (b.getName().equals(name)) if (b instanceof FunBind)
				...
				throw new java.lang.Error("duplicate bindings must be functions: " + name);
		}
		return new Binding(lineStart, lineEnd, path, name, type, type, mergeFunctions(name, fs));
	}
	...
	private static AST mergeFunctions(String name, Vector<FunBind> fs) {
		...
		if (arity == 0)
			...
			for (int i = 0; i < arity; i++) {
				args[i] = new Dec(fs.get(i).getLineStart(), fs.get(i).getLineEnd(), fs.get(i).getPath(), "$" + i, ast.types.Void.VOID, ast.types.Void.VOID);
				dummies[i] = new PWild();
				vars[i] = new Var(fs.get(i).getLineStart(), fs.get(i).getLineEnd(), "$" + i, ast.types.Void.VOID, null);
			}
			...
			for (int definition = 0; definition < fs.size(); definition++) {
				...
				arms[definition] = new BArm(patterns, fs.get(definition).getGuard(), fs.get(definition).getBody(), false);
			}
			arms[fs.size()] = new BArm(dummies, Bool.TRUE, new ast.control.Error(fs.get(0).getLineStart(), fs.get(0).getLineEnd(), new Str("ran out of options for " + name)), false);
			return new Fun(fs.get(0).getLineStart(), fs.get(0).getLineEnd(), fs.get(0).getPath(), new Str(name), args, fs.get(0).getType(),
			    new Case(fs.get(0).getLineStart(), fs.get(0).getLineEnd(), new Dec[] {}, vars, arms), false);
		}
	}
	...
	public static Env<String, Type> typeBindingsPar(Binding[] bindings, Env<String, Type> env) {
		...
		for (Binding valueBinding : valueBindings) {
			...
			Type dType = Type.eval(valueBinding.getDeclaredType(), env0);
			;
			...
			if (Type.equals(dType, vType, env0))
				...
				throw new TypeError(valueBinding.getLineStart(), valueBinding.getLineEnd(), "declared type for " + name + "::" + dType + " does not match the type of the value: " + vType);
		}
	...
	public static Env<String, Type> typeBindingsRec(Binding[] bindings, Env<String, Type> env) {
		...
		for (TypeBind typeBinding : typeBindings) {
			// This currently causes a problem when one type definition refers to an
			// imported type.
			// Fix in new version...
			// typeBinding.check(env);
		}
		...
		for (Binding valueBinding : valueBindings) {
			String name = valueBinding.getName();
			Type dType = Type.eval(valueBinding.getDeclaredType(), env);
			// dType.check(env);
			Type vType = valueBinding.type(env);
			if (!Type.equals(dType, vType, env))
			  throw new TypeError(valueBinding.getLineStart(), valueBinding.getLineEnd(), "declared type for " + name + "::" + dType + " does not match the type of the value: " + vType);
		}
	...
	public Binding(int lineStart, int lineEnd, String path, String name, Type declaredType, Type sourceType, AST value) {
		super(lineStart, lineEnd, path, name, declaredType, sourceType);
		this.value = value;
	}
	...
	public Dec[] getActBindingFields() {
		Dec[] args = getActBindingArgs();
		Dec[] bindings = getActBindings();
		int length = args.length + bindings.length;
		Dec[] fields = new Dec[length];
		for (int i = 0; i < args.length; i++) {
			fields[i] = args[i];
		}
		for (int i = 0; i < bindings.length; i++) {
			fields[args.length + i] = bindings[i];
		}
		return fields;
	}

	private Dec[] getActBindings() {
		Fun fun = (Fun) getValue();
	...
	public boolean isActBinding() {

		// The value of the binding is a behaviour definition.
		// Note that this should change to a specific AST element,
		// but is currently a function with a behaviour body...

		if (getValue() instanceof Fun) {
			Fun fun = (Fun) getValue();
			return fun.getBody() instanceof Act;
		} else
			return false;
	}
	...
	public Binding subst(AST ast, String name) {
		return new Binding(getLineStart(), getLineEnd(), path, this.name, getDeclaredType(), getSourceType(), value.subst(ast, name));
	}

	public Binding substExportedValues(Collection<Module> values) {
		return new Binding(getLineStart(), getLineEnd(), path, name, getDeclaredType(), getSourceType(), value.substExportedValues(values));
	}

	public String toString() {
		return "Binding(" + name + "," + value + "," + getDeclaredType() + ")";
	}

	public Type type(Env<String, Type> env) {
		Type declaredType = Type.eval(getDeclaredType(), env);
		Type actualType = Type.eval(getValue().type(env), env);
