...
@BoaConstructor(fields = { "name", "args", "declaredType", "sourceType", "body", "guard" })
...
public class FunBind extends Binding implements DecContainer {
	...
	public FunBind(int lineStart, int lineEnd, String path, String name, Pattern[] args, Type declaredType, Type sourceType, AST body, AST guard, boolean traced) {
		super(lineStart, lineEnd, path, name, declaredType, sourceType, null);
		this.args = args;
		this.guard = guard;
		this.body = body;
		this.traced = traced;
	...
	private Binding desugarPattern() {
		...
		for (int i = 0; i < args.length; i++) {
			s[i] = new Dec(getLineStart(), getLineEnd(), path, "$" + i, getArgType(i), getArgType(i));
			vs[i] = new Var(getLineStart(), getLineEnd(), "$" + i, getArgType(i), null);
			ws[i] = new PWild();
		}
		BArm a1 = new BArm(args, guard, body, false);
		BArm a2 = new BArm(ws, Bool.TRUE, new ast.control.Error(getLineStart(), getLineEnd(),
		    new BinExp(getLineStart(), getLineEnd(), new Str("ran out of case arms in " + name), "+", new List(getLineStart(), getLineEnd(), vs))), false);
		...
		return new Binding(getLineStart(), getLineEnd(), path, name, getType(),getType(),
		    new Fun(getLineStart(), getLineEnd(), path, new Str(name), s, getDeclaredType(), new Case(getLineStart(), getLineEnd(), new Dec[] {}, vs, new BArm[] { a1 }), traced));
	}

	private Binding desugarSimple() {
		return new Binding(getLineStart(), getLineEnd(), path, name, getType(), getSourceType(),new Fun(getLineStart(), getLineEnd(), path, new Str(name), simpleArgs(), getDeclaredType(),
		    new If(getLineStart(), getLineEnd(), guard, body, new ast.control.Error(getLineStart(), getLineEnd(), new Str("guard failed for " + name))), traced));
	}
	...
	private Dec[] simpleArgs() {
		...
		for (int i = 0; i < args.length; i++) {
			...
			s[i] = new Dec(getLineStart(), getLineEnd(), path, var.getName(), var.getDeclaredType(), var.getDeclaredType());
	...
	public Binding subst(AST ast, String name) {
		...
		if (bound.contains(name))
			...
			return new FunBind(getLineStart(), getLineEnd(), path, this.name, args, getDeclaredType(), getSourceType(),body.subst(ast, name), guard.subst(ast, name), traced);
	}

	public Binding substExportedValues(Collection<Module> values) {
		return new FunBind(getLineStart(), getLineEnd(), path, name, args, getDeclaredType(), getSourceType(),body.substExportedValues(values), guard.substExportedValues(values), traced);
	}

	private Type termAccessorType(Env<String, Type> env, Type[] types) {
		// The function binding is a term accessor. This can be compiled efficiently
		// when the
		...
		if (offset < MAX_TERM_ACCESSOR_OFFSET && getDeclaredType() instanceof ast.types.Fun) {
			...
			return new ast.types.Fun(getLineStart(), getLineEnd(), types, body.type(env));
	}
		...
		Pattern.types(args, env, (env1, types) -> {
			if (guard.type(env1) instanceof ast.types.Bool) {
				...
				if (isTermAccessor()) {
					...
					actualType = new ast.types.Fun(declaredType.getLineStart(), declaredType.getLineEnd(), types, body.type(env1));
				...
				if (Type.equals(actualType, declaredType, env1)) {
					...
					throw new TypeMatchError(getLineStart(), getLineEnd(), actualType, declaredType);
			} else
				throw new TypeError(guard.getLineStart(), guard.getLineEnd(), "expecting a boolean guard.");
		});
		return getType();
	}
