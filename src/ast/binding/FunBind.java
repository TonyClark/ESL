...
public class FunBind extends Binding implements DecContainer {
	...
	private Binding desugarPattern() {
		...
		BArm a2 = new BArm(ws, Bool.TRUE, new ast.control.Error(getLineStart(), getLineEnd(),
				new BinExp(getLineStart(), getLineEnd(), new Str("ran out of case arms in " + name), "+", new List(getLineStart(), getLineEnd(), vs))));
		...
		return new Binding(getLineStart(), getLineEnd(), path, name, getType(), new Fun(getLineStart(), getLineEnd(), path, new Str(name), s, getDeclaredType(),
				new Case(getLineStart(), getLineEnd(), new Dec[] {}, vs, new BArm[] { a1 })));
	}

	private Binding desugarSimple() {
		return new Binding(getLineStart(), getLineEnd(), path, name, getType(),
				new Fun(getLineStart(), getLineEnd(), path, new Str(name), simpleArgs(), getDeclaredType(),
						new If(getLineStart(), getLineEnd(), guard, body, new ast.control.Error(getLineStart(), getLineEnd(), new Str("guard failed for " + name)))));
	}
	...
	/*
	 * 
	 * public Type getFunctionType() { Type[] result = new Type[] { null };
	 * Env<String, Type> env = new Empty<String, Type>(); Pattern.types(args, env,
	 * (newEnv, types) -> { Type t = getType(); result[0] = new
	 * ast.types.Fun(getLineStart(), getLineEnd(), types, t); }); return
	 * result[0]; }
	 */
