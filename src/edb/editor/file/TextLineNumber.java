...
public class TextLineNumber extends JPanel implements CaretListener, DocumentListener, PropertyChangeListener {
  ...
  public void paintComponent(Graphics g) {
    ...
    while (rowStartOffset <= endOffset) {
      try {
        ...
        processLineNumber(g, fontMetrics, insets, rowStartOffset, availableWidth);
        // Move to the next row...
        rowStartOffset = Utilities.getRowEnd(component, rowStartOffset) + 1;
      } catch (Exception e) {
        // This seems to throw an exception when there is text wrapping.
        // However, the code seems to do the right thing, so we ignore the
        // exceptions...
        break;
      }
    }
  ...
  private void processLineNumber(Graphics g, FontMetrics fontMetrics, Insets insets, int rowStartOffset, int availableWidth) throws BadLocationException {
    ...
    if (lineNumber.length() > 0) {
      ...
      if (lineNumber.length() > 0 && num == selectedLine) {
        lineNumber = ">" + lineNumber;
        x = x - fontMetrics.stringWidth(">");
      }
      if (lineNumber.length() > 0 && breakpoints.contains(num)) {
        if (lineNumber.length() > 0 && num == selectedLine) {
          lineNumber = "*" + lineNumber;
          x = x - fontMetrics.stringWidth("*");
        } else {
