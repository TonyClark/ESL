...
esl = { 
  ...
  file(name)      -> es=exports iss=imports bs=bindings whitespace EOF { Module(name,es,Imports.merge(iss),bs) };
  ...
  actbind         -> [[ 'act' n=name gs=optGenericDecs [[ ps=actargs  t=typeDec d={Dec.generic(gs,FunType(FunType.types(ps),t))} ]] lcurl es=exports bs = bindings i=actinit as=barms rcurl  { Binding(n,d,Fun(Str.strAt0(n),ps,d,Act(Str.strAt0(n),ps,es,bs,i,as))) } ]];
  ...
  simpleType  -> [[ 'Int' { IntType() } ]]
              ...
              |  [[ 'KB' lcurl ts=namedRelations rcurl { KBType(ts) } ]]
