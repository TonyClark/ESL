export spec, specState

BRule       = 'ast.spec.BRule'.java()
Send        = 'ast.spec.Send'.java()
Spec        = 'ast.spec.Spec'.java()
State       = 'ast.spec.State'.java()
StateDef    = 'ast.spec.StateDef'.java()
...
spec = { 
  ...
  behaviour       -> whitespace 'behaviour' lcurl bs=behaviourRule* rcurl { bs };
  behaviourRule   -> sName=Name args=optArgs 'on' m=Name mArgs=optArgs b=('when' b=body | {[]}) arrow tName=Name tArgs=optArgs a=optAction { BRule(sName,args,m,mArgs,b,tName,tArgs,a) };
  optAction       -> 'do' lcurl as=action* rcurl { as } | {[]};
  action          -> [[ n=name eql v=val semi { Update(n,v) } ]] | [[ v=val leftArrow n=Name vs=optArgs { Send(v,n,vs) } ]];
