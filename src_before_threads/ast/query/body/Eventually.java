...
public class Eventually extends BodyElement {
  ...
  public Eventually(int lineStart, int lineEnd, BodyElement[] elements) {
  ...
  public BodyElement subst(Type type, String typeName) {
    ...
    return new Eventually(getLineStart(), getLineEnd(), newElements);
  }
  ...
  public void compile(Vector<Instr> code, int arity, Vector<String> vars, boolean isLast) {
    ...
    vars.add(time);
    code.add(new ast.query.instrs.temporal.Eventually(vars.indexOf(time) + arity));
    for (BodyElement e : elements)
