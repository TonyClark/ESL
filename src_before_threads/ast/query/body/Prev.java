...
public class Prev extends BodyElement {
	...
	public Prev(int lineStart, int lineEnd, BodyElement[] elements) {
	...
	public BodyElement subst(Type type, String typeName) {
		...
		return new Prev(getLineStart(), getLineEnd(), newElements);
	}
  ...
  public void compile(Vector<Instr> code, int length, Vector<String> vars, boolean isLast) {
    ...
    // We would like to be able to compile the body of prev so that it can perform last calls.
