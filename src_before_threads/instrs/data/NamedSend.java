...
public class NamedSend extends Instr {
  ...
  private void performSend(Actor actor, Actor target) {
    if (isIdle(target) && hasFunHandler(target)) {
      ESL.setWaiting(false);
      getMessageListener().send(ESL.getTime(), actor, target, name, arity);
      target.reset();
      target.openFrame(null, null);
      transferArgs(actor, target, 0);
      Closure closure = target.getFunHandler(name, arity);
      closure.apply(target, target, arity);
