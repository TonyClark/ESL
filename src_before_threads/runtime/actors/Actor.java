...
import static runtime.listeners.Listeners.instrListener;
import static runtime.listeners.Listeners.messageListener;
import static runtime.listeners.Listeners.newActorListener;
import static runtime.listeners.Listeners.scheduleListener;
...
public class Actor {
  ...
  int               frame               = -1;
  int               openFrame           = -1;
  boolean           recentlyHandledTime = false;
  int               instructions        = 0;
  int               localTime           = 0;
  ...
  int               currentFail         = -1;
  int               tofs                = 0;
  ActorQueryMachine machine             = null;
  int               line;
  ...
  public boolean canGrab(int n) {
    ...
    for (int i = 0; i < n; i++)
      if (!canGrab((Grabbable) peek(i))) return false;
    return true;
  ...
  public int qShow(Clause clause, int timeoutAddress, int altAddress, int instrs) {
    ...
    //
    // This method may be called in three modes:
    // (1) The first time the actor tries to perform SHOW.
    // (2) When the previous SHOW terminated (success or fail).
    // (3) When the previous SHOW exhausted its allocated instructions.
    //
    // In the case of (3) there will be no data on the stack for the machine.
    ...
    if (machine == null) {
      ...
      machine = new ActorQueryMachine(rules, knowledge, this, instrs, timeout);
    } else {
  ...
  public synchronized void queueMessage(String name, Object[] values) {
    ...
    synchronized (ESL.getMonitor()) {
      ESL.getMonitor().notifyAll();
    }
  }
  ...
  public Object run(int instrs) {
    ...
    try {
      ...
      boolean active = (!complete() || !messageQueue.isEmpty()) && instrs > 0;

      while (active) {
        if (complete()) dequeueMessage();
        ...
        switch (next.code) {
          ...
          case SHOW:
            ...
            instrs = qShow(showInstr.getClause(), showInstr.getTimeoutAddress(), showInstr.getAltAddress(), instrs);
            break;
          case STARTCALL:
            openFrame(null, null);
            break;
        ...
        if (ESL.isStop() || state == ActorState.BLOCKED) instrs = 0;
        instrs--;
        active = (!complete() || !messageQueue.isEmpty()) && instrs > 0;
      }
  ...
  public void scheduleMessage() {
    ...
    // message Time(time) and adjust the local time. Otherwise, if there is a
    // message pending on the queue then deliver it...

    if (ESL.getTime() > localTime && behaviour.handlesTime()) {
      localTime = ESL.getTime();
      if (recentlyHandledTime && !messageQueue.isEmpty())
        dequeueMessage();
      else {
        if (complete() && hasFunHandler("Time", 1)) {
          openFrame(null, null);
          pushStack(ESL.getTime());
          recentlyHandledTime = true;
          getFunHandler("Time", 1).apply(this, this, 1);
        } else {
          if (complete()) {
            Term term = new Term(Time, ESL.getTime());
            recentlyHandledTime = true;
            processMessage(term);
          } else queueMessage("Time", new Object[] { ESL.getTime() });
        }
      }
    } else dequeueMessage();
  }
